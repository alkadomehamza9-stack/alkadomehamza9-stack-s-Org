  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    for (const key in formData) {
      if (formData[key as keyof typeof formData]) {
        studentData[key] = formData[key as keyof typeof formData];
      }
    }
    
    const newStudent = await supabaseAPI.addStudent(studentData);
    setIsSubmitting(false);

    if (newStudent) {
      onStudentAdded(newStudent);
      setFormData({ name: '', grade: GRADES[0], class: '', national_id: '', birth_date: '', address: '', phone1: '', phone2: '', email: '', parent_name: '' });
      onClose();
    } else {
      ...
      <form onSubmit={handleSubmit}>
        ...
        <div className="mt-6 flex justify-end gap-3">
          ...
          <button type="submit" disabled={isSubmitting} className="bg-purple-600 text-white py-2 px-4 rounded-lg hover:bg-purple-700 disabled:bg-purple-300 flex items-center">
            {isSubmitting ? 'جاري الحفظ...' : <><Icon name="save" className="w-4 h-4 ml-2" /> حفظ الطالب</>}
          </button>
        </div>
      </form>
    </Modal>
